...
extension SearchResultsViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configure(with: row.poster_path ?? "")
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        APIManager.shared.getMovie(with: titleName) { [weak self] result in
            ...
            case .success(let videoElement):
                self?.delegate?.searchResultsViewControllerDidTapItem(DetailViewModel(title: title.original_title ?? "", youtubeView: videoElement, titleOverview: title.overview ?? ""))
